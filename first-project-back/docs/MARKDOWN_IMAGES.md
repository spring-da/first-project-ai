...
- 意见/回复图片、头像和 Markdown 图片共用前端 64 MiB、最多 64 张、最长 1 小时的内存缓存。同 URL 的并发读取合并为一次下载；组件关闭只取消自己的订阅，最后一个订阅关闭才中止网络请求。退出、重新登录或切换成员工作区会清理缓存和未完成请求，旧会话的迟到响应不会进入新缓存。此缓存不写入浏览器持久存储。
- 意见及回复图片上传成功后直接用本机文件填入预览缓存，避免立即再次通过服务器下载。图片下载不计入顶部页面同步进度条，意见及回复中的图片仍保留加载状态和失败重试；显式重试会移除该图片的内存缓存并重新请求服务器，避免重复读取损坏字节。公开分享图片不进入此缓存，重新打开时必须再次校验分享有效期和撤销状态。
...
图片实际经过“OSS → 应用服务器 → 浏览器”两段传输。Bucket 设置为公共读不会绕过这条链路，服务器出口带宽、地域距离和原图大小都可能限制首次加载速度；仅凭加载动画无法判断哪一段慢。

1. 在浏览器 Network 筛选 `image` / `user-avatars`，查看 Size、Timing、Protocol；只记录大小和耗时，不复制携带 Authorization 的请求或含分享 Token 的 HAR。`Queueing/Stalled` 长表示浏览器连接或并发等待；`Waiting (TTFB)` 长时继续排查后端鉴权、数据库和 OSS 首字节；`Content Download` 长时关注图片体积和服务器到用户的带宽。
2. 对比首次打开、同一页面再次打开和刷新后的请求数。应用内再次打开应命中内存缓存；刷新后依赖后端图片接口的 private 缓存头。浏览器开发者工具的 Disable cache 会影响 HTTP 缓存验证，不代表正常访问效果。
3. 在服务器端用 OSS 控制台/监控核对 Bucket 地域、标准存储、GetObject 延迟和返回字节；确认后端实际运行环境的 Endpoint 与地域相符。同地域阿里云内网 Endpoint 只适用于支持的网络，其他厂商或跨地域服务器不要直接改成 internal。
4. 看服务器出口带宽是否已满。例如 2 MiB 原图通过 2 Mbit/s 出口，仅传输的理论下限就约 8 秒，多图并发还会分享带宽。当前意见图片上传前会压缩，但旧图片不会因此变小；若首次访问主要卡在大原图下载，应再实施缩略图/原图分离或增加带宽，并保留现有鉴权边界。

本地隔离测试只能验证请求合并、缓存、加载状态和会话隔离；真实 OSS 首次下载延迟需在部署环境按以上步骤测量。
