...
- 一次性邀请链接注册、邮箱登录及 BCrypt(12) 密码散列
- 管理员人员管理、模拟登录成员完整工作区、短期随机临时密码、JWT 会话撤销和登录失败锁定
- JWT 无状态认证
- 按登录用户隔离所有业务数据
...
└── db/migration/  # V1 基础结构、V2–V8 增量迁移
```
...
后续版本会继续按顺序执行增量迁移，其中 V7 引入管理员人员管理，V8 加固一次性邀请、临时密码、登录锁定和 JWT 会话撤销，V9 扩展任务日期、截止时间、优先级、完成时间与归档状态。
